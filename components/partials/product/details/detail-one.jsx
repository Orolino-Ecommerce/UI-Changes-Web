...
function DetailOne(props) {
  ...
  const [microdataKey, setMicrodataKey] = useState(0);
  ...
  useEffect(() => {
    ...
    if (variantFromUrl) {
      console.log("inside varianturl");
      const colorCode = variantFromUrl.slice(-6); // Get the last 6 characters as color code
      const size = variantFromUrl.slice(0, -6);
      console.log("variantss", colorCode, size);
      // Find the variant in the variationGroup that matches the color code and size
      const matchingVariant = variationGroup.find(
        (variant) => variant.color === `#${colorCode}` && variant.size === size
      );
      console.log("variationGroup", variationGroup);
      if (matchingVariant && matchingVariant !== lastSelectedVariant) {
        console.log("matchingVariantInnnn");
        setSelectedVariant({
          color: matchingVariant.color,
          colorName: matchingVariant.colorName,
          price: matchingVariant.price,
          Orgprice: matchingVariant.Orgprice,
          size: matchingVariant.size,
          variantId: matchingVariant.variantId,
          productId: matchingVariant.productId,
        });
        setLastSelectedVariant(matchingVariant);
        onColorClick(matchingVariant.color);
        getOutOfStocks(
          product?.product_id,
          ...
          items: [
            {
              ...
              product_variant_prices: [
                {
                  product_variant_size_detail: matchingVariant.size,
                  product_color_code: matchingVariant.color,
                  product_variant_price_mrp: matchingVariant.price,
                },
              ],
            },
          ],
        ...
        trackCustomEvent("pixel_view-content", commonViewEventData);

        setMicrodataKey((prevKey) => prevKey + 1);
      }
    }
  }, [router.query.variant, wishlistData, variationGroup, lastSelectedVariant]);
  ...
  const checkWishList = (wishlistData, productColor, productSize) => {
    ...
    console.log("wishlistDataInside====", wishlistData);
  ...
  // console.log("checkWishListnormal", checkWishList);
  //buynow
  ...
  function onWishlistClick(e) {
    ...
    if (userId) {
      if (
        ...
        trackCustomEvent("pixel_add-to-wishlist", commonWishEventData);
      } else {
        toast.error("Product with this variant already added to your wishlist");
      }
    } else {
  ...
  function onCartClick(e, index = 0) {
    ...
    trackCustomEvent("pixel_add-to-cart", commonCartEventData);
    // } else {
  ...
  function updateURLWithVariant(color, newsize) {
    ...
    const updatedPath = `/bd/products/${product?.product_name
      .trim()
      .replace(/\s+/g, "-")
      .toLowerCase()}/${product?.product_id}-${product?.product_code
      .trim()
      .replace(/\s+/g, "-")
      .toLowerCase()}/?variant=${stringWithoutHash}`;
  ...
  //   selectedVariant?.size
  // );
  console.log("microdataKey", microdataKey);
  return (
    <>
