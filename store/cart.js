...
export const actionTypes = {
  ...
  removeFromCartClear: "CLEAR_FROM_CART",
...
const cartReducer = (state = initialState, action) => {
  switch (action.type) {
    case actionTypes.addToCart:
      ...
      if (
        findIndex !== -1 &&
        action.payload.product?.product_variant_prices?.length > 0
      ) {
    ...
    case actionTypes.removeFromCartClear:
      return {
        data: [
          ...state.data.filter((item) => {
            if (item.id !== action.payload.product.id) return true;
...
export const actions = {
  ...
  removeFromCartClear: (product) => ({
    type: actionTypes.removeFromCartClear,
    payload: {
      product: product,
    },
  }),
...
export function* cartSaga() {
  ...
  yield takeEvery(actionTypes.removeFromCartClear, function* saga(e) {
    //toast.success("Product removed from Cart");
  });
