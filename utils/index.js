...
export const canAddToCart = (cartItems, product, qty) => {
  // console.log("cartttttt", product);
  // console.log("cartItems", cartItems);
  // console.log("qty", qty);
  let find = cartItems.find((item) => item.product_id == product.product_id);
  if (find) {
    // console.log("ffff");
    if (product.product_variant_prices?.outofstock == 1) return false;
    else return true;
  } else {
    // console.log("dddd");
    if (product.product_variant_prices?.outofstock == 1) return false;
...
export const isInWishlist = (
  ...
  // console.log("isInWishlist", product_color_code, product_variant_offer_price);
  return (
    product &&
    wishlist.findIndex((item) => item.product_id == product.product_id) > -1
  );
};
...
export const catFilter = function (products = [], category, flag = false) {
  ...
  return products.filter((item) => {
    ...
    for (let i = 0; i < category.length; i++) {
      if (item?.subcategory?.subcategory_name == category[i]) {
        if (!flag) return true;
      } else {
...
export const catDealFilter = function (products = [], category, flag = false) {
  if (category[0] === "All") return products;

  return products.filter((item) => {
    for (let i = 0; i < category.length; i++) {
      if (item?.product?.subcategory?.subcategory_name == category[i]) {
        if (!flag) return true;
      } else {
        if (flag) return false;
      }
